...
#  GROQ — TEXT
# ══════════════════════════════════════════════════════════════
def call_groq(chat_id, user_message, system_override=None):
    add_to_history(chat_id, "user", user_message)
    system   = system_override or SYSTEM_PROMPT
    messages = [{"role": "system", "content": system}] + get_history(chat_id)
    ...
    for attempt in range(4):
        try:
            ...
            if r.status_code == 200:
                ...
                return "❌ Groq API key invalid. Update GROQ_API_KEY in config."
            else:
                log.error(f"Groq {r.status_code}: {r.text[:200]}")
                return f"⚠️ Groq error ({r.status_code}). Try again."
    ...
    return "⚠️ Groq is busy right now. Please try again in a moment."
    ...
    for attempt in range(3):
        try:
            ...
            if r.status_code == 200:
                ...
                return call_groq(
                    chat_id,
                    f"The user sent an image with caption: '{user_prompt}'. "
...
def handle_generate_doc(chat_id, topic, doc_type="pdf"):
    ...
    content = call_groq(chat_id, prompt, system_override=system)
